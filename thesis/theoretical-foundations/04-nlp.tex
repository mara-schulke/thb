...
extraction, capturing meaning and structure. For NL2SQL systems NLP enables
...
Before neural networks can process text, the raw text must be converted into
structured representations. This transformation is called tokenization, which
refers to the technique of segmenting text into discrete units called tokens.
...
For NL2SQL systems, tokenization must be capable of both encoding NL queries
and SQL queries. SQL presents a challenge in NLP as it relies on a structured
syntax and usage of special characters (e.g., punctuation, underscores, SQL
operators) which are less commonly found in natural language.

\subsubsection{Word Embeddings and Semantic Representations}

Word embeddings are the vector space representation of tokens where semantic
...
semantic properties such as semantic distance. Early embedding methods like
Word2Vec and GloVe learned static representations where each word has a single
vector regardless of its surrounding context. Modern contextual embeddings,
generate different vectors for the same word based on its context, thus
capturing meaning more accurately.
...
Different pooling strategies emerged, like mean pooling, max pooling, using a
CLS token or learned aggregation. \Natural~uses an embedding model
(Qwen3-Embedding-4B) trained specifically for generating high-quality sentence
representations for subsequent searching.
...
direction input vectors. This satisfies properties desirable for distance
