...
For example, for $\omega_{ground}$, the question ``Give me all contacts for the user
...
and~\ref{figure:schema-denormalized}, the respective definitions of
...
WHERE id = 10;
    \end{verbatim}
    \vspace{1.3em}
    \caption{Using array access on a denormalized database schema in SQL.}
...
The distance index $d$ contains precomputed distances between observed
database schemas, which enables efficient retrieval during example
