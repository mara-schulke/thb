...
where $q \in \mathcal{Q}$ is the input natural language query, $C \in \mathcal{C}$
is the current set of generated query candidates, $s \in \mathcal{S}$ is the database schema,
...
\begin{enumerate}
    ...
    \item \textbf{SQL similarity} – Structural similarity between the current best
        candidate $\hat{\omega} = \nu(C)$ and candidate SQL queries using code
        representations, enabling pattern recognition across different database
        domains. On the first iteration $C = \emptyset$ and this term is omitted.
...
where $C \in \mathcal{C}$ is the set of generated query candidates so far, $s \in \mathcal{S}$
is the full database schema, and $s' \subseteq s$ is the reduced schema subset containing
only relevant tables and columns.

The function analyzes the generated candidates in $C$ to identify which schema elements
are referenced across them. On the first iteration $C = \emptyset$ and the full schema
$s$ is returned unchanged. This enables
