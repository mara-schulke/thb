...
Prior to the widespread dominance of machine learning approaches for NLP
...
Foundational research of NL2SQL systems mostly focused around applying rule
...
information that was derived from processing the NLQ. \citep{Rendezvous, Lunar,
Ladder}. These approaches mostly tried to translate NLQs into formal grammars
which could then be deterministically mapped into a valid SQL query
\citep{Lunar}. These approaches have a limited number of language patterns they
can process and require adaptation for new or unknown databases, query patterns
etc. A potential upside of this class of NL2SQL systems is that they can
confidently and reproducibly identify questions they can, and can't answer —
thus leading to reliable and predictable user interfaces.
...
the database. This architecture led to better transferability across databases
...
In \citeyear{NALIR} \citeauthor*{NALIR} highlighted that a perfect translation
of natural language into SQL was challenging as natural language is not made
for query expressions and heavily relies on contextual information and
clarifying questions in order to disambiguate in conversations. These learnings
...
translate all of them in natural language for the user to choose from''
\citep{NALIR}.
...
Many of the outlined approaches faced severe challenges when moving outside
of research environments. Frequently systems performed comparatively well on
research benchmarks that were often comprised of controlled question types and
limited data variety. Ultimately no standard benchmarks existed for NL2SQL in
...
\begin{enumerate}
    \item \textbf{Limited language coverage} — Traditional NL2SQL systems were
        only able to process a small subset of natural language constructs
    ...
    \item \textbf{Brittleness} — Legacy systems did not handle synonyms,
        paraphrasing, or spelling errors. Manual adaptation and handling was
        needed to remediate. \citep{StringKernels, SQLizer}.
    \item \textbf{Poor scalability} — With potentially more complex underlying
        databases, traditional solutions often performed worse.
...
\cite{Rendezvous}), NLIDBs were and are not considered to be a solved problem.
