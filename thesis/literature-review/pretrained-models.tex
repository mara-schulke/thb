...
language models for NLIDBs. Models like BERT or T5 offer noticeable performance improvements (especially when it
comes to language understanding) over specialized NL2SQL networks due to training happening on unrestrained amounts
of natural language data, instead of pure NL2SQL datasets which are often fairly limited in size and therefore natural
language use — \textsc{Spider2.0} which is a contemporary NL2SQL benchmark consists of just 632 real-world questions
\citep{Spider2}. Thus PLM-based (or at least augmented) NL2SQL systems can observe dramatic performance improvements
...
In the time of emerging PLM application \textsc{Grappa} was introduced by \citeauthor*{GRAPPA} in \citeyear{GRAPPA} —
...
which analyses and identifies patterns in natural language queries that can be used as templates for synthesizing training
...
/ domains which was an apparent problem with neural approaches which had a strong tendency to overfit on benchmark datasets. 
...
approaches at the time \citep{RYANSQL}. The sketch-based approach makes \textsc{Ryansql} a PLM-augmented successor of 
...
Following the advancements through T5, \citeauthor*{GRAPHIX} introduced GRAPHIX-T5 in \citeyear{GRAPHIX}, which combined
...
of T5 models as well as the database schema structure during inference. GRAPHIX-T5 constructs a schema graph where nodes
...
robustness gain decoupled architectures have over model-oriented approaches.
