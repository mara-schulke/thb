...
synthetic training data using a Synchronous Context-Free Grammar which
...
\textsc{StruG}, a structure-grounded pre-training approach, was subsequently
introduced in \citeyear{STRUG} by \citeauthor*{STRUG} and presented a
pretraining approach that improves model abilities for schema linking,
separating the problem in three facets: column grounding, value grounding and
column-value mapping. \textsc{StruG} achieves similar performance to
\textsc{Grappa} while being cheaper to train \citep{STRUG}.
...
\textsc{Spider} \citep{PICARD}.

This addresses an issue associated with PLMs — while they outperform in natural
language understanding, they often lack SQL grammar knowledge and tend to
generate queries that are not executable due to unconstrained output space
\citep{PICARD}.
...
Research on applying PLMs for NL2SQL primed the field for transition towards
LLM usage. While PLMs range from millions to a few billion parameters,
