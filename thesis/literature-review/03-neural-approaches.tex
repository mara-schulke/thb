...
understanding and accuracy were shown, a larger paradigm shift took place in
...
order-sensitivity of sequence-to-sequence neural networks harmed accuracy. 
...
(test) \citep{SQLNet}. \textsc{TypeSQL} incorporated type-aware encoding of
inputs, further improving performance by 4.7\% above \textsc{SQLNet} while
reducing the required training time by half \citep{TypeSQL}. 

\textsc{SyntaxSQLNet} improved the ability of NL2SQL systems on complex
...
representation (IR) format (SemQL) and approached NL2SQL with three-phase
processing: schema linking, IR synthesis using SemQL, and deterministic SQL
translation \citep{IRNet}. Notably, graph neural networks (GNNs) were explored
for neural network representation of database schemas. \textsc{GNN}
...
novel approach for solving schema linking \citep{RATSQL}.

RAT-SQL's primary innovation was the ability to infer, understand and utilize
...
actual database schema while generating the query. This yielded a 57.2\% in
exact match accuracy when being evaluated on the \textsc{Spider} benchmark,
substantially outperforming comparative approaches like GNN, IRNet and IRNet V2
by 10.5\%, 9.8\% and 8.7\% respectively. Although overall accuracy improved
across all approaches when being paired with BERT (Bidirectional Encoder
Representations from Transformers, a popular pre-trained language model from
Google) the $\Delta$ between the individual approaches remained steady. RAT-SQL
outperformed SOTA approaches by 5\% to 12.2\% which demonstrated its capability
advancement \citep{RATSQL}.

\subsubsection{Comparative Analysis}

Neural approaches progressively improved upon each other in multiple dimensions:

\begin{enumerate}
    \item \textbf{Model Sophistication} — From sequence-to-sequence
        ...
        demonstrated increasing architectural sophistication in order to
        improve execution accuracy on benchmarks like \textsc{Spider}
        \citep{Seq2SQL, SQLNet, TypeSQL, GNN, SyntaxSQLNet, RATSQL}.
    ...
    \item \textbf{Query Complexity} — The performance on complex queries
        ...
        at-the-time capability ceiling on complex queries using neural
        approaches, outperforming \textsc{IRNet} by 10.5\% \citep{IRNet,
        RATSQL}.
    \item \textbf{Schema Understanding} — \textsc{TypeSQL}, \textsc{GNN} and
        \textsc{RAT-SQL} each demonstrated enhanced schema linking
        capabilities, yielding the ability to accurately reason about user
        intent and traverse the database schema during query generation
...
suffered from serious shortcomings and unsolved challenges:
...
\begin{enumerate}
    ...
    \item \textbf{Domain Language} — Despite the increased transferability
        of neural approaches, they still had a limited vocabulary and
    ...
    \item \textbf{Observability} — The black-box nature of neural networks made
        ...
        neural network architectures, hard to understand and explain in cases
