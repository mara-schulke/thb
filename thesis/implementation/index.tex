...
This section discusses the implementation phase of \textsc{Natural},
translating the system design outlined in Section \ref{section:design}
into a working NL2SQL system. Engineering challenges encountered, performance
bottlenecks identified, and optimization strategies implemented to achieve
deployment viability on consumer hardware are discussed and examined.
Furthermore, the section covers software architecture and infrastructure and
implementation of each pipeline component.
...
\ref{section:design}, sampling code focuses on indexing samples and computing
distance indices, and evaluation code runs the \textsc{Natural} pipeline on
benchmarks. This subsection outlines the scope of each component, the design
rationale, and technology stack decisions.
...
quantized version of the 8B model consumes 8GB of VRAM which cannot fit
into 24GB when accounting for KV-Cache requirements. Instead of
choosing a heavily quantized version of the 8B variant (i.e., Q4\_KM or
below), the 4B model with Q8\_0 shows similar performance on simple use cases
while offering a smaller memory footprint at 2GB \citep{Qwen3Embed}.
...
velocity. This section analyzes crucial technology decisions for \Natural,
discussing trade-offs between research velocity and production readiness,
performance optimization and development speed, and ecosystem maturity vs.
cutting-edge capabilities.

Critical decisions include the programming language, model inference framework,
and vector search implementation. Each decision was evaluated against limited
hardware resources, database integration capabilities, and reproducible
research outcomes.
...
and other general purpose languages. While interpreted languages like Python, R
and Julia tend to be easier to use for rapid prototyping and validation due to
their loose type systems and scientific ecosystem, they come with drawbacks
regarding deployability, speed, and robustness compared to compiled and
strongly typed languages. Languages like Java, C / C++ and Rust offer stability
at runtime, better interoperability into other programs (e.g., database
extensions) and resource utilization they represent a trade-off between
performance optimization and portability vs. iteration speed and research
ecosystems.
...
Candle, even though it offered comparable inference speeds, required extensive
low-level implementation work and proved incompatibility with scenarios where
the binary is embedded into a database (e.g., PostgreSQL). The combination of
manual tensor management, convoluted quantization support, and deployment
constraints made llama.cpp a better alternative for production-ready systems.
...
The development overhead of choosing Rust over Python for the implementation
was noticeable; Rust's machine learning and natural language processing
frameworks are less advanced, porting research code written in Python from
other papers (like \cite{OmniSQL}) turned out to be non-trivial. This decision
likely doubled the time for implementation, but provides a clear path for the
algorithms to be productionized. The implementation has significantly better
performance and portability than Python would have allowed.
...
distance of the question embedding. Structural SQL similarity is computed using
the cosine distance of the SQL embedding, and schema distance using WWL kernel
distance from the \texttt{natural-graphs} component. The embeddings are
computed using the Qwen3-Embedding-4B model using the embedding
implementation in \texttt{natural-sampling}.
...
(question, query, structural), the implementation employs constant weights
which were empirically determined: 70\% for semantic question similarity and
30\% for SQL structural similarity. The final scoring combines 70\%
sample-level similarity with 30\% schema-level compatibility. The selection
algorithm yields a maximum of 256 candidates (\texttt{TOP\_K = 256}) to limit
computational overhead whilst maintaining selection quality. Vector similarity
queries are performed using SQLite with \texttt{sqlite-vec}, achieving
sub-second retrieval times for vector databases exceeding 2,000,000 samples.

% TODO: REWRITE THIS TO HERE

\paragraph{Implementation Architecture}

The \texttt{Selector} struct encapsulates embedding computation and representation
from the selection algorithm. Via \texttt{Selector::new} the code can compute
the embeddings and use them to run example selection.
...
% TODO: Add explanation for why 0.5

\texttt{ICLGenerator::generate} implements few-shot learning with relevance
filtering. Relevance filtering refers to removing selected samples with a
similarity of less than 0.5 which was empirically derived from
evaluations. Thus only semantically or structurally similar samples are
provided to the model. The prompt is constructed based on an adapted version of
\textsc{OmniSQL} format \citep{OmniSQL}: task overview, sql schema, filtered
examples with similarity scores, explicit instructions, and target question.
...
(see Appendix~\ref{appendix:prompt:natural:inference}) compared to DAIL-SQL. Key
differentiations include the encoding of similarity scores to give the model
the ability to weight samples.

This prompt steers the model towards precision and COT reasoning.
...
\texttt{natural-inference::pipeline::refinement}. This implementation
...
context rather than loading two separate models. This design choice
significantly reduces memory requirements but potentially impacts the
refinement capabilities compared to having a dedicated quality assurance model
with different prompt conditioning. Due to the limited available hardware the
effects of having a separate, distinct refinement model could not be verified.
\citeauthor{XiYan} have implemented multi model generation pipelines in
\citeyear{XiYan} and achieved promising results.

\paragraph{Output Processing and Validation}

Similar to ICL generation, the refinement responses are processed through the
same \texttt{Markdown} parser to extract the predicted SQL. The system
validates each extracted candidate by trying to parse it using
\texttt{QueryCandidate::try\_from} and returns the first syntactically valid
...
generation. It processes the \texttt{RawQueryCandidate} and provides a
self-correction mechanism that improves overall pipeline robustness. Notably
both the unrefined and the refined query candidates are kept in the candidate
set for majority voting.
...
While the file system layout of \Spider~and \Bird~is largely similar but not
identical.

This abstracted benchmarking system (see Appendix~\ref{appendix:benchmark-traits})
allows integrating further benchmarks in the future (e.g., \textsc{Spider2} which
...
effort compared to Python. Using Python would have provided immediate access to
existing machine learning infrastructure and could have leveraged existing
implementations by previous researchers directly in the implementation of
\Natural. The mature Python ecosystem for NLP and ML eliminates the need to
develop abstractions for model inference, vector database operations, and
...
than anticipated, requiring extensive development and research for
functionality that would be available off-the-shelf Python packages. The
integration with Python libraries for specialized components like the
...
concerns. Despite these challenges, the resulting architecture demonstrates
that multi language approaches can be viable when different components have
distinct requirements. The Python integration was isolated to the sampling
phase, preserving the runtime portability characteristics of the core Rust
...
outweigh the benefits of compiled languages like Rust.

For projects with a clear path to production deployments or those
...
be justified. Overall, the availability of hardware impacts the implementation
timeline significantly more than language chosen.

Implementations in this field should follow a two-staged approach:
prototyping and algorithm validation in Python to leverage existing tools and
speed up iteration, followed by a production port in a systems language like
Rust once the algorithmic approach is validated and stable.
