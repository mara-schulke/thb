...
large language models like GPT-4 and Claude, these systems introduce
data privacy and security concerns for real world adoption: Database
schemas and queries must be transmitted to an external API for
inference. This incurs risks, ongoing costs and a
...
locally on consumer hardware. The following research questions are
addressed:
...
State-of-the-art systems combine multiple NLP and ML techniques like
in-context learning, self-correction and candidate selection. They rely
...
whether the gap between proprietary systems and open-source models
like \textsc{OmniSQL}-7B can be reduced or closed by combining the models with
other NLP or ML techniques. Specifically:
...
To resolve the dependency on proprietary systems for NL2SQL, a
cost-effective local deployment must be achieved. An offline NL2SQL system must
utilize compute efficiently as combining all available techniques
may not be optimal for performance characteristics. If components have
dependencies, redundancies, or unfavorable cost-benefit ratios the
