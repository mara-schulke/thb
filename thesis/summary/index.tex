...
\begin{enumerate}
    \item \textbf{Graph-Based Example Selection} — A new schema-aware example
...
that open-source models can achieve competitive NL2SQL performance in
controlled environments. During local evaluations, systematic pipeline
composition with graph-based example selection showed to be the critical
performance driver, especially on complex databases. Pipeline components show
synergistic rather than additive tendencies, but further evaluations,
especially better ablations are required to make an absolute assessment.
...
especially the schema-aware example selection, show that additional
