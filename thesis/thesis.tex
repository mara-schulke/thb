...
it comes to fusing discrete parsing approaches using a formal grammer like LSNLIS developed by \cite{Lunar} and 
...
\citeauthor*{SQLizer} show that alternating between quantitative repair and synthesis yields results that beat
state-of-the-art approaches like NaLIR. SQLizer is fully automated and database-agnostic, requiring no knowledge
of the underlying schema. The authors evaluated SQLizer on 455 queries across three databases, where it ranked the
correct query in the top 5 results for about 90\% of cases, significantly outperforming NaLIR \citep{NALIR},
