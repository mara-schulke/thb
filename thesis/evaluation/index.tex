...
28.1\% on \Bird~(dev). The section performance gap section
(\ref{sec:eval:performance-gap}) analyzes this gap in detail.
...
only a minimal performance degredation of -2.6pp. \Spider~(test) which is not
...
external knowledge is showing an extreme degredation of -28.1pp.
...
This subsection is analzing potential causes using available evidence.
...
\Bird) substantially exceeed the typical effects of model format conversions
indicating that other factors might contribtue to the observed performance loss.
...
pharsing, layout or presentation could yield widely diverging results. It is
likely that \OmniSQL~7B was fine-tuned using a specific prompt format, and deviation
from this format can significantly degrade performance.
...
Appendix~\ref{appendix:prompt:omnisql:inference}. \Natural's prompt deviated from
the presumed original format through the addition of similarity scores for
in-context learning examples, more explicit instructions and the likely modified
...
was performed. This singificantly limits the ability to draw
...
which is conistent with the evaluation environment and results.
...
Furthermore component ablations in the evaluation provid valid evidence for
...
NL2SQL research community would suggest from standardization of inference
...
performance tradeoffs. This would allow for emperical decision making when
...
prevalent benchmarks. Conistent results across the frameworks would further
increase the confidence in reproducibility of results where descrepancies would
