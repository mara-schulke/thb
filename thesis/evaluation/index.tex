...
benchmarking are \OmniSQL-7B and \textsc{Qwen3 Embedding 4B Q8} for generation
...
The evaluation of \Natural~revealed five primary insights:

\begin{enumerate}
    \item \textbf{Relative Improvements} – \Natural~\textit{Full} achieves
        higher scores than the baseline configration and models throughout.
        \Natural~achieves 81.0\% \EA~on \Spider (dev), 81.4\% on \Spider~(test)
        and 53.8\% on \Bird~(dev), compared to \OmniSQL-7B-gguf with
        79.0\% on \Spider~(dev and test) and 38.0\% on \Bird.
...
published and the locally measured performance of \OmniSQL~7B (using GGUF with
F16 weights) and the reported accuracy metrics results from \cite{OmniSQL}.
...
(dev). These results further fall below the published \OmniSQL-7B
...
are predominantly semantic rather than syntactic, which validates the
robustness of \OmniSQL's SQL generation capabilities.
...
splits respectively). This poor baseline performance confirms \Bird~being a
significantly more challenging benchmark. While the \textit{Zero-Shot}
...
on \Spider~(test) and 0.2\% on \Bird~(dev).
...
achieves minimal candidate latency (7.3s on \Spider~dev, 7.3s on test,
9.3s on \Bird~dev), representing the time required for schema
...
\textit{Syn} configurations achieve 15s-17s \CL~on \Spider~and 33s-44s on
\Bird, with the \textit{Syn} configuration yielding the highest latency
(43.7s on \Bird).
...
These characteristics inform hardware requirements and deployment decisions.
For interactive applications sub-second response times of these models are
recommended. \Natural~clearly exceeds these thresholds by a factor of up to 45
on complex databases and questions (e.g., \Bird). Thus either significantly
better hardware is required, a simpler configuration needs to be used or
further performance optimizations need to be introduced.
...
ICL examples, more explicit instructions and the likely modified schema representation.
...
be observed (eg, \textit{Syn} achieving 53.8\% vs. \OmniSQL~7B-gguf achieving
...
absolute performance of \Natural. Multiple factors influence the the observed
