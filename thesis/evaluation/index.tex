...
Section~\ref{lit:benchmarks:bird}). The complex \Spider~2.0 version of \Spider~was
...
Measurements on \Bird, which is designed to represent real-world databases with
imperfect data (see Section~\ref{lit:benchmarks:bird}), external knowledge
requirements and hard to reason about database schemas yields significantly
lower absolute performance numbers but shows improvements through ICL based
systems. While the \textit{Baseline} configuration of \Natural~only achieves
32.4\% \EA~(less than half the accuracy than on \Spider~with 77.9\% and 77.0\%
for dev and test splits respectively). This performance confirms that \Bird~is
a significantly more challenging benchmark. While the \textit{Zero-Shot}
...
which were reported to have an \EA~of 66.1\% and 61.5\% respectively. The
...
aware example selection, self-refinement, majority voting) no clear statement
can be made on the absolute performance of \Natural. A multitude of differences
cn influence the observed gap, including the model format, inference
implementation, hyperparameter, prompt template, and benchmark version used.
Nonetheless, the relative comparisons within the local evaluation environments
remain reliable, therefore enabling valid, but thesis-local conclusions.
