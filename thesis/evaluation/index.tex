...
The evaluation of \Natural~builds utilizes the benchmarking infrastructure
implemented in Section~\ref{impl:benchmark}. This section is evaluating the
performance metrics of different \Natural~configurations on two prevalent
benchmarking datasets: \Spider~and \Bird. A representative baseline is
introduced, aiming to mirror the base-model performance. Subsequently an
ablation study is performed to understand the performance impact of individual
pipeline components.
...
inference on GPUs and Rust 1.91 for compiling \Natural. The models used during
benchmarking are \OmniSQL-7B and Qwen3 Embedding 4B (Q8) for generation
...
Comparable research publishes the execution accuracy and exact match metrics on
these benchmarks which enables relative and absolute performance comparisons of
\Natural~with other studies.

\subsubsection{Evaluation Metrics}\label{eval:metrics}
...
Execution Accuracy (or \EA) is the primary success metric of NL2SQL systems as
it represents the semantic accuracy of the SQL queries produced by NL2SQL
systems. \EA is computed by determining whether the rows in the candidate
results are a permutation of the rows returned by ground truth results. 

For a query $q_g$ with candidate results $R_c$ and ground truth results $R_g$, 
where $R_c, R_g \subseteq \mathbb{V}^{n \times m}$ (sets of $n$ rows with $m$
columns of values $\mathbb{V}$), semantically accurate execution is defined as:
...
candidate and ground truth SQL queries after syntax normalization. Compared to
\EA, which validates semantic correctness through result comparison,
\EM~determines whether the generated query structurally matches the ground
truth query. \EM~thus acts as a lower bound for execution accuracy as queries
must be identical which is therefore more restrictive as semantically
equivalent queries with different syntax (e.g., using a different join order)
...
To determine the performance impact of methods and components
...
crucial components) and external baselines (e.g., proprietary system
...
representing the performance contribution of \OmniSQL~as closely as
...
pipeline components activated but without any examples available to use during ICL.
...
\Spider~and \Bird~but covers a wide array of domains through
...
splits from \Spider~and \Bird~\citep{Spider, BIRD}. The train
...
used during evaluation (dev and test) from \Spider~and \Bird~
\citep{Spider, BIRD}. These allow \textsc{Natural} to reference correct answers
as examples during generation indicating the potential upper-limit of
performance during a self-learning deployment which has access to past
conversations.
...
(\textit{Baseline}, \textit{Zero-Shot}, as well as different vector DBs)
against external prior art (GPT-4, \OmniSQL). Subsequently performance
characteristics and improvement trends are quantitatively and qualitatively
analyzed.

\subsubsection{Overview}

% TODO: Table or table, Figure or figure? Unify.
The following sections present these findings across benchmarks and
configurations in detail. The system metrics are presented in
Table~\ref{tab:eval:overall-results} and visualized in
Figures~\ref{fig:eval:execution-accuracy-overview}
and~\ref{fig:eval:exact-match-overview}, which displays the performance for all
system configurations across the \Spider~and \Bird~datasets, the execution
accuracy (\EA), exact match (\EM), error rate (\ER) and candidate latency (\CL)
metrics.
...
\begin{figure}[htbp]
  ...
  \caption{
    % TODO: Omit benchmarks without values
...
published and the locally measured performance of \OmniSQL~7B-gguf (with F16
...
(dev). These results further fall below the published \OmniSQL-7B performance
of 81.6\%, 89.8\%, and 66.1\% respectively, representing gaps of 3.7pp, 12.8pp,
and 33.7pp. This continued degradation of performance indicates that a
misaligned prompt template (i.e., divergent from training phase) harms
performance if not paired with counter measures such as ICL, self-correction or
majority voting. These performance numbers mark the \textit{Baseline}
configuration the worst performing variant of \Natural~on \EA~and \EM~across
\Spider~(dev and test) and \Bird~(dev).

% TODO: Has subsetting been critized?
...
Figures~\ref{fig:appendix:spider-dev-ea}, \ref{fig:appendix:spider-test-ea},
\ref{fig:appendix:spider-dev-em}, and~\ref{fig:appendix:spider-test-em}).
...
query refinement contribute minimal performance gains if any. Notably,
...
\begin{figure}[htbp]
  ...
  \caption{
    ...
    on test) compared to \textit{Baseline} (7.3s) without notable performance
    gains, while ICL configurations incur additional overhead from example
    retrieval.
  }
...
+21.4\% on \Spider~(test). These double digit improvements in accuracy, as shown
in Figure~\ref{fig:eval:baseline-improvement-ea}, point
towards the conclusion that ICL is an effective mechanism for further improving
the performance of already fine-tuned models. The performance
...
(see Appendix~\ref{sec:appendix:benchmark-results-spider}).
...
\begin{figure}[htbp]
  ...
  \caption{
    ...
    improvements peaking at +21.4pp (\textit{Syn} on \Bird~dev). The substantial
...
\begin{figure}[htbp]
  ...
  \caption{
      ...
      that failures are predominantly semantic rather than syntactic or runtime
      related.
  }
...
per hundred queries), see Figure~\ref{fig:eval:error-rates}, indicating that
failures are predominantly semantic rather than syntactic, which validates
the robustness of \OmniSQL's SQL generation capabilities.

\subsubsection{BIRD Results}

% TODO: Maybe refer to the lit review?

Measurements on \Bird, which is designed to represent real-world
databases with imperfect data, external knowledge requirements and hard to
reason about database schemas yields significantly lower absolute performance
numbers but shows improvements through ICL based systems. While the
\textit{Baseline} configuration of \Natural~only achieves 32.4\% \EA~(less than
half the accuracy than on \Spider~with 77.9\% and 77.0\% for dev and test
splits respectively). This performance confirms that \Bird~is a significantly
more challenging benchmark. While the \textit{Zero-Shot} configuration shows
incremental improvements over the baseline with a relative improvement of
+1.6pp in \EA~(34.0\% absolute), the overall performance shows to be behind the
reported numbers of \OmniSQL-7B and GPT-4 by \cite{OmniSQL} which were
reported to have an \EA~of 66.1\% and 61.5\% respectively. The locally
reproduced measurement of \OmniSQL-7B-gguf yielded a drop in \EA~of -28.1pp
compared to the officially reported \EA~yielding 38.0\% in absolute accuracy.
...
This shows a notable contrast with \textsc{Spider}, where synthetic examples
...
The other metrics \EM, \ER~and \CL~further show a substantial difference between
\Spider~and \Bird. Systems which mostly relied on the model~(like \Natural~
...
(see Figure~\ref{fig:appendix:baseline-improvement-em} for detailed \EM~improvements).
...
for \textit{Ground}, as illustrated in Figure~\ref{fig:eval:candidate-latency-bird}.
...
55.8\% \EA, representing a +23.4pp improvement over \textit{Baseline}, a +72\%
relative gain. Despite these substantial relative improvements within
...
performance with 64.0\%. This notable difference between local replication and
...
low error rates: \textit{Ground} shows an \ER~of 0.7\% on \Spider~ (dev), 0.3\%
on \Spider~(test) and 0.2\% on \Bird~(dev). This consistency indicates that the
\OmniSQL~model generates syntactically valid, executable SQL with a high
reliability, and that pipeline components do not introduce significant failure
modes. Furthermore these low error rates reveal an important characteristic of
\OmniSQL~and \Natural: Errors of these NL2SQL systems are predominantly
semantic rather than syntactic or runtime-related. This distinction matters for
production deployments, as semantic errors are significantly harder to validate
and catch than hard system failures. Semantic accuracy might be further
improved through user feedback which can feedback into ICL. 
...
achieves minimal candidate latency (7.3s on \Spider~(dev and test) and 9.3s on
\Bird~dev), representing the time required for schema
subsetting and SQL generation without example retrieval or refinement.

The \textit{Zero-Shot} pipeline configuration approximately doubles the
end-to-end latency to (14.0s, 15.3s, 18.7s respectively) highlighting that the
...
selection and ICL show further latency increases. The \textit{Train} and
\textit{Syn} configurations achieve 15.4-16.3s \CL~on \Spider~and 33.7-44.7s
on \Bird, with the \textit{Syn} configuration yielding the highest latency
(43.7s on \Bird).
...
for the $\pi$ component of \Natural. These characteristics inform hardware
requirements and deployment decisions. As shown in
Figure~\ref{fig:eval:latency-accuracy-tradeoff}, the trade-off between latency
and accuracy varies significantly across configurations and benchmarks. For interactive applications
sub-second response times of these models are recommended. \Natural~clearly
exceeds these thresholds by a factor of up to 45 on complex databases and
questions (e.g., \Bird). Thus either significantly better hardware is
required, a simpler configuration needs to be used or further performance
optimizations need to be introduced.
...
\begin{figure}[htbp]
  ...
  \caption{
    ...
    (configuration, benchmark) pair. This figure highlights that moving from
    \textit{Baseline} to ICL-enabled configurations increases \EA~, but yields
    significant higher \CL, particularly on \Bird~with a \CL of 43s. The
    \textit{Zero-Shot} configuration represents an inefficient operating point
    with a comparatively high latency to accuracy.
  }
...
discrepancy emerged (see Figure~\ref{fig:eval:omnisql-gap}) between the locally
...
\begin{figure}[htbp]
  ...
  \caption{
    ...
    local reproduction and published results.
  }
...
\begin{table}[ht]
    ...
    \end{tabular}
    \caption{
        Performance gap between published \OmniSQL~7B results \citep{OmniSQL}
        and local measurements using \OmniSQL-7B-gguf. Negative values indicate
        underperformance relative to published baselines.
    }
...
queries is showing a moderate degradation of -10.8pp. \Bird~(dev) which is the
most complex benchmark of the three, featuring larger schemas, dirty data and
external knowledge is showing an extreme degradation of -28.1pp. This is
indicating a performance drop correlating with task difficulty, suggesting that
the locally measured model had impaired reasoning capabilities, difficulties
handling larger schemas or handling external knowledge requirements.

\subsubsection{Potential Contributing Factors}

Several factors may contribute to this notable gap between the two systems.
This subsection is analyzing potential causes using available evidence.

\paragraph{Model Format Differences}

One apparent difference between the published performance metrics and the local
measurements for \OmniSQL~7B is the model format and precision used. The
...
preprocessing variations could contribute to divergent results. The $\Delta$ of
2.6\% on \Spider~(dev) is well within the expected format-induced degradation
ranges, which initially suggests that the model format alone could explain the
simple benchmark discrepancy. However, the widening performance gaps with
increasing benchmark complexity, up to 28.1pp on \Bird, substantially exceed
the typical effects of model format conversions indicating that other factors
might contribute to the observed performance loss.

% TODO: Double check this section

\paragraph{Inference Implementation Differences}

\citeauthor{OmniSQL} likely used vLLM as their inference engine, while
local measurements of \OmniSQL~7B used Rust FFI bindings to llama.cpp. These
engines implement the same mathematical operations but may differ
in the implementation of matrix operations, attention, sampling algorithms and
KV-cache management. To keep further inference environment divergent as small
as possible, similar (where possible) sampling configurations, prompts and
hyperparameters where used.
...
A prompt drift could cause significant performance differences the exact prompt
as supplied by \citeauthor{OmniSQL} was used. Subtle differences in phrasing,
layout or presentation could yield widely diverging results. It is
likely that \OmniSQL-7B was fine-tuned using a specific prompt format, and
deviation from this format can significantly degrade performance. The local
benchmarking implementation used the published prompt template but likely had a
slightly diverging code representation implementation than the authors of
\OmniSQL. The code representation implementation used by the benchmarking
harness is based around the \texttt{sqlparser} crate in Rust which includes an
implementation for human readable formatting of SQL queries. While these
modifications are subtle, they may inadvertently misalign with the model's
learned expectations on the code representation format, confusing the
generation process.

The \textit{Baseline} configuration results are further evidence for a possible
prompt drift: Even when using only the inference logic ($\pi$) without
additional pipeline components, \Natural~achieved 77.9\% EA on \Spider~(dev)
compared to OmniSQL-7B-gguf's 79.0\%, a 1.1pp degradation despite using the
same model and quantization. This suggests that even \OmniSQL~is noticeably
sensitive to prompt template differences affect performance. The prompt
template used in the local benchmarks for \Natural~is shown in
...
ICL examples, more explicit instructions and the likely modified schema
representation.
...
performance gains are excellent (e.g., \textit{Syn} achieving 53.8\% vs.
\OmniSQL-7B-gguf achieving 38.0\% on \Bird) but given that no clear absolute
improvement could be measured (e.g., \textit{Syn} achieving 53.8\% vs
\OmniSQL-7B achieving 66.1\% on \Bird). Furthermore the comparisons with other
external systems become unclear if local measurements face a significant gap in
...
not infect the validity results measured between different variants of
\Natural~ and between \Natural~and \OmniSQL. All \Natural~configurations and
...
showed an improvement of +21.4pp over \textit{Baseline} on \Bird~which was
...
(66.1\% \EA) and local measurements (38.0\% \EA) represents a challenge for
deriving absolute conclusions. While strong incremental performance gains could
be observed (e.g., \textit{Syn} achieving 53.8\% vs. \OmniSQL-7B-gguf achieving
38.0\% on \Bird) by applying the techniques outlined in this thesis (schema
aware example selection, self-refinement, majority voting) no clear statement can be made on the
absolute performance of \Natural. Multiple factors influence the observed
performance gap, including the model format, inference implementation,
hyperparameter, prompt template and dataset version used. Nonetheless, the
relative comparisons within the local evaluation environments remain
reliable, therefore enabling valid, but thesis-local conclusions.
